    ...
    try:
        while camera.isOpened():
            try:
                ...
                image_resized = img_pil.resize(input_size(interpreter), Image.ANTIALIAS)
                input_tensor = np.array(image_resized, dtype=np.uint8)
                interpreter.set_tensor(interpreter.get_input_details()[0]['index'], [input_tensor])
                interpreter.invoke()
                results = get_objects(interpreter, min_confidence)
                ...
                if results :
                    for obj in results:
                        ...
                        #cv2.rectangle(frame, (int(box.xmin), int(box.ymin)), (int(box.xmax), int(box.ymax)), boxLineColor, LineType)
                        cv2.rectangle(frame, (int(box.xmin), int(box.ymin)), (int(box.xmax), int(box.ymax)), boxColor, lineType)
