...
class ObjectsOnRoadProcessor(object):
    ...
    def __init__(self,
        ...
        self.input_width, self.input_height = input_size(self.interpreter)
    ...
    def detect_objects(self, frame):
        ...
        # Convert frame to RGB and resize to match model input
        # call tpu for inference
        #start_ms = time.time()
        frame_RGB = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        #img_pil = Image.fromarray(frame_RGB)
        img_pil = Image.fromarray(frame_RGB).resize((self.input_width, self.input_height), Image.ANTIALIAS)
        
        # Run interference
        #self.interpreter.set_tensor(self.interpreter.get_input_details()[0]['index'], [np.array(img_pil)])
        ...
        if objs:
            for obj in objs:
                ...
                logging.debug(f"{label}, {score:.0f}% bbox={bbox}")
                ...
                # Draw bounding box
                coord_top_left = (int(bbox.xmin * self.width), int(bbox.ymin * self.height))
                coord_bottom_right = (int(bbox.xmax * self.width), int(bbox.ymax * self.height))
                cv2.rectangle(frame, coord_top_left, coord_bottom_right, self.boxColor, self.boxLineWidth)
                ...
                # Annotate text
                annotate_text = f"{label} {score:.0f}%"
                cv2.putText(frame, annotate_text, (coord_top_left[0], coord_top_left[1] + 15), self.font, self.fontScale, self.boxColor, self.lineType)
